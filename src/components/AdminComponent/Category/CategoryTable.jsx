...
import { addOffer, getCategory } from "@/api/Admin/categoryApi";
...
const CategoryTable = () => {
  ...
  const { blockOrUnblockCategory,offerAdd, categoryEdit, setCategories } =
    useCategory();
  ...
  const handleOfferSubmit = async (categoryId, offerValue) => {
    if (offerValue.trim() === "" || isNaN(offerValue) || Number(offerValue) < 0) {
      toast.error("Enter a valid offer percentage");
      return;
    }
            ...
            <tbody>
              {categories.map((category, index) => (
                <tr key={category._id} className="border text-center">
                  ...
                  <td className="p-3 border">{category.offer || 0}%</td>
                  <td className="p-3 border">
                    <Dialog>
                  ...
                  <td className="p-3 border">
                    <div className="flex justify-center items-center space-x-2">
                      <Dialog
                        open={!!selectedEditCategory}
                        onOpenChange={(open) =>
                          !open && setSelectedEditCategory(null)
                        }
                      >
                        <DialogTrigger asChild>
                          <button
                        ...
                        <DialogContent className="sm:max-w-[425px] bg-white rounded-2xl shadow-xl p-6">
                          ...
                          <Button
                            onClick={() =>
                              handleEditSubmit(selectedEditCategory._id)
                            }
                            className="mt-4"
                          >
                            Save
