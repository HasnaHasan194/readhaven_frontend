...
const ItemOrderDetails = () => {
  ...
  const getCurrentStatusIndex = (status, lastValidStatus) => {
    ...
    if (formattedStatus === "cancelled" && lastValidStatus) {
      return statusSteps.findIndex(
        (s) => s.toLowerCase() === lastValidStatus.toLowerCase()
      );
    }
  ...
  // Define return eligibility function - can be placed at the top of your component
  const isReturnEligible = (order) => {
    // Only delivered orders can be returned
    if (order.itemDetails.status !== "Delivered") {
      return false;
    }
    ...
    // Return is eligible if within 7 days of delivery
    return daysDiff <= 7;
  };

  // Calculate days left for return - can be used in your UI
  const getDaysLeftForReturn = (order) => {
    if (order.itemDetails.status !== "Delivered") {
      return 0;
    }
  ...
  const refundAmount = Math.round(
    order.itemDetails
      ? order.itemDetails.productPrice *
          order.itemDetails.quantity *
          (1 - effectiveDiscountRate)
      : 0
  );
  ...
  const handleUpdatePaymentStatus = async (status) => {
    ...
    try {
      const response = await axiosInstance.put(`/users/orders/${orderId}/payment-status`, {
        status
      })
      await fetchOrderDetails();
  ...
  console.log(
    `discounted amount derive:${discountAmountDerived} effectiveDiscount rate: ${effectiveDiscountRate} refund amount:${refundAmount}`
  );
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-4 py-8 bg-gray-50 min-h-screen">
            ...
            return (
              <div key={status} className="flex-1 relative">
                {index < statusSteps.length - 1 && (
                  <div
                    className={`absolute top-6 left-1/2 right-0 h-1 ${
                      isCompleted
                        ? "bg-black"
                        : order.itemDetails.status === "Cancelled" &&
                          index === statusSteps.length - 1
                ...
                <div className="flex flex-col items-center relative z-10">
                  ...
                  <p
                    className={`text-xs mt-1 text-center max-w-[120px] ${
                      isCompleted
                        ? "text-black"
                        : isCurrent
                        ? "text-black"
      ...
      {/* Return Policy Notice - NEW COMPONENT */}
      {order.itemDetails.status === "Delivered" && (
        <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl border border-indigo-100 p-4 mb-8 shadow-sm">
              ...
              <p className="text-sm text-indigo-700">
                ...
                {isReturnEligible ? (
                  <span className="font-medium">
                    {" "}
                    You have {getDaysLeftForReturn} day
                    {getDaysLeftForReturn !== 1 ? "s" : ""} left to return this
                    item.
                  </span>
                ) : (
                  <span className="font-medium">
      ...
      <div className="mt-6 flex justify-end">
        {order.paymentStatus == "Failed" && (
          <RetryComponent total={order.totalAmount} updatePaymentStatus={handleUpdatePaymentStatus} />
        )}
        {/* cancel button */}
        {(order.itemDetails.status === "Pending" ||
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 mt-8">
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="md:w-1/3 flex justify-center">
            <img
              src={
                order.itemDetails.product.productImages?.[0] ||
                "/placeholder.svg"
              }
          ...
          <div className="md:w-2/3 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <span className="block text-lg font-medium text-gray-800">
                  {formatCurrency(
                    order.itemDetails.productPrice * order.itemDetails.quantity
                  )}
                </span>
              </div>
            </div>
